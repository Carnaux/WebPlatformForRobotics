...
loader.load(
	...
	function ( geometry, materials ) {
		...
		function constdata() {
			if(boolm1){
				pivot1.rotation.z -= 1.5; // cada sinal são 0,25° do motor
				boolm1 = false;
			} else if(boolm1R){
				pivot1.rotation.z +=  1.5;
				boolm1R = false;
			}
			if(boolm2){
				if(volt == 90){
...
//------------------AJAX-------------------------
		...
		function Page() {
			...
			mosq.onmessage = function(topic, payload, qos){
				...
				if(payload[0]+payload[1]+payload[2] == "TM1"){
					var tot = payload[3]+payload[4]+payload[5];
					if(tot == 080){
						this.volt = 80;
